...
| 모델 | 요청/일 | 토큰/분 | 봇 1개 연속 가동 |
|------|--------:|--------:|------------------|
| `llama-3.1-8b-instant` (채택) | 14,400 | 6,000 | 약 20시간 |
| `openai/gpt-oss-20b` | 1,000 | 8,000 | **83분** |
| `llama-3.3-70b-versatile` | 1,000 | 12,000 | **83분** |

6초 주기 = 10콜/분. 프롬프트는 약 340토큰(POI 3개 + 사람 1명) → **약 4,000 TPM**. 한도 6,000이라
여유가 크지 않다. **방이 2개가 되거나 주기를 줄이면 넘는다.** 프롬프트에 필드를 추가할 때도 마찬가지.

8b를 고른 건 품질이 아니라 이 한도 때문이다. 데모 직전에 70b로 바꿔보는 건 yml 한 줄이지만,
그날 하루 테스트를 83분 안에 끝낼 수 있을 때만.
