...
final class Interactables {
    ...
    private static final List<Poi> ALL = List.of(
            ...
            // (화장실 쪽지 note-med1·note-laundry1은 2026-08-05 사용자 지시로 제거. 자물쇠
            //  hint와 같은 규칙만 적혀 있었고 설명 대상과 25m 떨어진 방에 있었다.
            //  연병장 쪽지 note-laundry2·note-med2는 2026-08-03에 같은 이유로 제거.
            //  프론트 interactables.ts도 동일 — 이 목록은 프론트와 짝을 맞춰야 한다.)
            // ⚠️ 이제 화장실에는 봇이 들를 POI가 없다. 봇 순회 코스에서 화장실이 빠지는 것은
            //    의도된 결과다(빈 방이라 갈 이유가 없다).
            // 감시탑 각인(정문 코드 힌트). 연병장 개활지라 직선으로 닿는다.
            new Poi("gate-note1", -34, -27, false, false, "서쪽 감시탑 각인(연병장, 남서 벤치 앞)"),
            new Poi("gate-note2", 39, -28.5, false, false, "동쪽 감시탑 각인(연병장, 남동 구석)"),
            // ── 별관(2026-08-01 추가) ───────────────────────────────────────────
