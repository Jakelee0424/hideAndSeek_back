 ...
 *
 * <h2>왜 A* 격자가 아닌가</h2>
 * 맵이 손으로 만든 고정 구조라 통행 가능한 위상이 뻔하다(감방 → 복도 → 출입구 →
 * 연병장/정문). 개활지가 넓어 대부분 직선(LOS)으로 풀리고, 노드는 벽을 돌아야 하는
 * 출입구만 잇는다. 노드 17개면 충분하고, 격자 A*보다 훨씬 싸고 디버깅도 쉽다.
...
final class BotNav {
    ...
    /** 통행 가능 지점(도면 배치). 인덱스가 EDGES의 참조 번호다. 복도·문처럼 벽을 돌아야 하는
     *  지점만 잇고, 연병장 개활지(단서·탈옥문)는 대부분 직선(LOS)으로 닿는다.
     *  맵: 수감동(북서, 복도 z=17) ↔ 연결 복도 ↔ 별관(북동) — 남쪽 출입구 → 연병장 → 정문. */
    private static final Node[] NODES = {
        new Node(-30, 24),   // 0  감방 1-1 안(북측)
        new Node(-14, 24),   // 1  감방 1-2 안(북측)
        new Node(-30, 10),   // 2  감방 1-3 안(남측)
        new Node(-14, 10),   // 3  감방 1-4 안(남측)
        new Node(-30, 19),   // 4  수감동 복도 서 북측 통로(1-1 문 앞 — 서쪽 끝 계단이 z 16~18을 막는다)
        new Node(-14, 17),   // 5  수감동 복도 동(1-2·1-4 문 앞)
        new Node(0, 17),     // 6  연결 복도(철창·화장실·출입구 교차점)
        new Node(14, 17),    // 7  별관 복도 서(식당·작업장 문 앞)
        new Node(30, 17),    // 8  별관 복도 동(세탁실·의무실 문 앞)
        new Node(14, 24),    // 9  식당 안
        new Node(30, 24),    // 10 세탁실 안
        new Node(14, 10),    // 11 작업장 안
        new Node(30, 10),    // 12 의무실 안
        new Node(0, 10),     // 13 단지 출입구 밖(두 동 사이 골목)
        new Node(0, -12),    // 14 연병장 중앙
        new Node(0, -26),    // 15 교도소 정문(탈옥문)
        new Node(-30, 15),   // 16 수감동 복도 서 남측 통로(1-3 문 앞)
        new Node(0, 24),     // 17 화장실 안(연결 복도 북쪽 문으로 들어간다)
    };

    private static final int[][] EDGE_PAIRS = {
        {0, 4}, {2, 16}, {1, 5}, {3, 5},  // 감방 안 → 문 앞 복도(문 통과)
        // 복도 서쪽 끝의 계단(z 16~18, 난간벽 전 높이) 때문에 서편은 북(4)/남(16) 통로로 나뉜다.
        // 둘은 계단 동쪽의 노드 5를 거쳐 만난다.
        {4, 5}, {16, 5}, {5, 6}, {6, 7}, {7, 8}, // 복도(수감동 → 철창 → 별관)
        {7, 9}, {8, 10}, {7, 11}, {8, 12}, // 별관 복도 → 각 방(문 통과)
        {6, 13}, {13, 14}, {14, 15},      // 출입구 → 연병장 → 정문
        // 흩어 놓은 쪽지 하나가 화장실에 있다. 노드만 더하면 고립된다 — 간선은 여기 명시해야 한다
        // (인접은 lineClear로 자동 계산하지 않고 이 목록이 전부다).
        {6, 17},                          // 연결 복도 → 화장실(문 x=0 통과)
    };
