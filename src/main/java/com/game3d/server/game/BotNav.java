 ...
 *
 * <h2>왜 A* 격자가 아닌가</h2>
 * 맵이 손으로 만든 고정 구조라 통행 가능한 위상이 뻔하다(본관 감방 → 운동장 개활지 →
 * 식당/정문). 개활지가 넓어 대부분 직선(LOS)으로 풀리고, 노드는 벽을 돌아야 하는
 * 출입구만 잇는다. 노드 15개면 충분하고, 격자 A*보다 훨씬 싸고 디버깅도 쉽다.
...
final class BotNav {
    ...
    /** 통행 가능 지점(도면 배치). 인덱스가 EDGES의 참조 번호다. 개활지라 대부분 직선(LOS)으로
     *  움직이고, 이 노드는 감방 출입·식당 입구처럼 벽을 돌아야 하는 지점만 잇는다.
     *  (운동장 단서·탈옥문은 개활지라 노드 없이 LOS로 닿는다.) */
    private static final Node[] NODES = {
        new Node(-59, 36),   // 0  감방 1-1 안
        new Node(-59, 26),   // 1  감방 1-1 문 밖(남향)
        new Node(-43, 36),   // 2  감방 1-2 안
        new Node(-43, 26),   // 3  감방 1-2 문 밖
        new Node(-27, 36),   // 4  감방 1-3 안
        new Node(-27, 26),   // 5  감방 1-3 문 밖
        new Node(-11, 36),   // 6  감방 1-4 안
        new Node(-11, 26),   // 7  감방 1-4 문 밖
        new Node(-40, 10),   // 8  본관 앞 서편 개활지
        new Node(0, 4),      // 9  운동장 중앙
        new Node(0, -30),    // 10 운동장 남부(정문 방향)
        new Node(0, -48),    // 11 교도소 정문(탈옥문)
        new Node(38, 4),     // 12 별관 앞 동편 개활지
        new Node(40, 39),    // 13 식당 문 밖
        new Node(54, 39),    // 14 식당 안
    };

    private static final int[][] EDGE_PAIRS = {
        {0, 1}, {2, 3}, {4, 5}, {6, 7},   // 감방 안 → 문 밖(남)
        {1, 3}, {3, 5}, {5, 7},           // 감방 앞 열 연결
        {1, 8}, {7, 9}, {8, 9},           // 감방 앞 → 운동장
        {9, 10}, {10, 11},                // 운동장 → 남부 → 정문
        {9, 12}, {12, 13}, {13, 14},      // 운동장 → 별관 앞 → 식당(문 통과)
    };
